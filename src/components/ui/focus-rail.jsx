...
// Simple class name merger (shadcn-style helper)
function cn(...classes) {
...
export function FocusRail({
  ...
  return (
    <div
      className={cn(
        "group relative flex h-[600px] w-full flex-col overflow-hidden bg-[#BB2929] text-white outline-none select-none overflow-x-hidden",
        className
      )}
          ...
          {visibleIndices.map((offset) => {
            ...
            return (
              <motion.div
                key={absIndex}
                className={cn(
                  "absolute aspect-[3/4] w-[260px] md:w-[300px] rounded-2xl border-t border-white/20 bg-white shadow-2xl transition-shadow duration-300",
        ...
        <div className="mx-auto mt-12 flex w-full max-w-4xl flex-col items-center justify-between gap-6 md:flex-row pointer-events-auto">
          <div className="flex flex-1 flex-col items-center text-center md:items-start md:text-left h-32 justify-center">
            <AnimatePresence mode="wait">
              ...
              >
                {activeItem.meta && (
                  <span className="text-xs font-medium font-EireneSansRegular uppercase tracking-wider text-white">
                    {activeItem.meta}
                  </span>
                )}
                <h2 className="text-3xl tracking-tight md:text-4xl font-KuraleRegular text-white">
                  {activeItem.title}
                </h2>
                {activeItem.description && (
                  <p className="max-w-md text-neutral-400 font-EireneSansRegular">
                    {activeItem.description}
                  </p>
                )}
              </motion.div>
          ...
          <div className="flex items-center gap-4">
            ...
            {activeItem.href && (
              <a
                ...
                className="group flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-semibold text-[#BB2929] transition-transform hover:scale-105 active:scale-95"
              >
                Explore
